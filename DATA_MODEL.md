...
Fields: `entry_id`, `season`, `manager_name`, `team_name`, `points_this_week`, `points_gross`, `transfer_cost`, `points_total`, `transfers_made`/`transfers_remaining`, `active_chip`, `bank`, `value`, `gw_winner`, `last_synced`, `data_version`, and (only on backfilled rows, see below) `active_chip_backfilled` (BOOL).
...
Unlike the `points` bug, this data is NOT permanently lost for past seasons -- it's not FPL's season-bound `/event/{gw}/live/` endpoint (which stops serving old-season data once a new season exists), it's each manager's own historical picks record, which FPL keeps accessible indefinitely. `lambda/fpl-data-ingester/scripts/backfill-active-chip.mjs` re-fetches `/entry/{id}/event/{gw}/picks/` for every existing row and corrects `active_chip` where it differs, marking backfilled rows `active_chip_backfilled: true`.
