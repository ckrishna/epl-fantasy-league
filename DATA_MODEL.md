...
- **Tests:** `tests/group-seasons.test.mjs` (new, 5 tests, mirrors `tests/league-groups.test.mjs`'s structure) plus `tests/trends.test.mjs`'s mock helper and its two league-scoping tests updated to mock `group_seasons` instead of `leagues`. `tests/league-groups.test.mjs` (testing the now-unimported `utils/league-groups.mjs`) is left as-is for now — task #127 covers deciding what happens to it, together with task #128's decision on the `leagues` table itself.
