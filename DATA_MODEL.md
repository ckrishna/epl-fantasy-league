...
- Captain Pick, Chip Watch, and Differential Pick are still the original hand-written mock content, identical for every manager -- not a gap in the transfer algorithm itself, but worth flagging so nobody assumes the whole Advisor modal is real now. Chip Watch has one real-data guard (used_chips, above) against its single most obviously-wrong failure mode; Captain Pick and Differential Pick have none yet.

**Frontend wiring:** `getSquadAdvisor(entryId, gw)` (`src/api/client.js`) mirrors `getManagerSquad`'s convention of throwing on a non-OK response rather than silently returning fake data. `AdvisorModal` (`ManagerSquad.jsx`) fetches it on open and renders a `found: false` response as its own honest "nothing to suggest right now" row (with a reason-specific message) rather than treating it as an error. The dev-only `/__advisor-preview` route (`mockSquad` prop, no real `entryId` to query) skips the fetch entirely and falls back to `MOCK_ADVISOR`'s placeholder transfer instead, so that page still has something to render. `suggestTransfer`/`handleSquadAdvisor`/`getUsedChips` are covered by `lambda/stats-api/tests/squad-advisor.test.mjs`.
