...
**Manager picker is scoped to the current season (added 2026-08-12):** `/trends/managers` only returns names that have at least one row in the current season -- a manager who only played, say, 2019/20-2022/23 and isn't in this year's league shouldn't clutter a picker meant for "your trends." Their full history is still reachable directly via `/trends?manager=...` if ever needed; this filter only affects what shows up in the dropdown, not what `handleTrends` itself will return for a given name.
