...
**Historical 2025/26 data is not retroactively fixed by this** — the fix only changes what gets written going forward. Whether the ~6,337 already-ingested rows can be backfilled depends on whether FPL's `/event/{gw}/live/` endpoint still serves last season's per-gameweek data now that 2026/27 exists (unconfirmed as of this writing — element IDs are also not guaranteed stable across seasons, which would break any such backfill even if the endpoint responds).
