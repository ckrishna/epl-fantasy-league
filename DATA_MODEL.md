...
erDiagram
    ...
    fpl_entry_picks {
        ...
        string position_player "SK: player_id (name is legacy, see fix below)"
        boolean is_captain
    }
...
### `fpl_entry_picks`
Partition key `season_entry_gw` (S, `"{season}#{entry_id}#{gw}"`), sort key `position_player` (S, the player's element id alone, e.g. `"501"` — **not** `"{squad_position}#{player_id}"` despite the attribute's name; see the duplicate-row bug below for why it was changed and why the attribute wasn't renamed). 6,337 items (as of before the 2026-09-08 dedupe cleanup below — see that entry for the corrected count).
...
**Bug (found + fixed 2026-09-08): duplicate rows per player, caught live via a manager's squad view showing several players twice** — once as a starter, once on the bench (Gvardiol, Cunha, Lewis-Potter, Haaland, João Pedro, Havertz all duplicated for one manager; Mosquera and Coyle appeared as both a starter AND a bench "sub" with identical stats in both places). Root cause: this table's sort key used to be `position_player: "{pick.position}#{pick.element}"`. `gwsToFetch` in `fpl-data-ingester/index.mjs` re-polls `activeGW - 1` through `activeGW` on every scheduled run (hourly during live windows, see the scheduling-split entry above) specifically so post-auto-substitution scores settle in over time — meaning the SAME gameweek's picks get fetched repeatedly from FPL. Between two of those polls, FPL's own auto-substitution processing (or a manager reordering their bench before a gameweek locks) can change a player's `position` for an ALREADY-STORED gameweek. Because `PutCommand` only overwrites an item when BOTH the partition key AND sort key match exactly, a changed `position` meant a changed sort key — the OLD row was never found and overwritten, just left behind as an orphaned duplicate alongside the new one, silently, for as long as the corruption had been happening.

Fixed by changing `position_player` to `String(pick.element)` — stable per player regardless of how many times `position` legitimately changes for the same (season, entry, gameweek, player). The attribute name `position_player` was deliberately left as-is rather than renamed, to avoid an unrelated second churn on top of this fix. `squad_position` (a separate, non-key field on the same item) still gets updated to whatever FPL reports each poll — only the *key* needed to stop moving. Proven via `tests/picks-duplicate-key.test.mjs`: two separate `handler()` runs for the same gameweek, with the same player reported at a different `position` each time, now produce an identical sort key both times (a real `PutCommand` would overwrite, not duplicate).

Already-corrupted production rows need a one-time cleanup, since the fix only stops NEW duplicates from forming — it doesn't retroactively merge ones already written. `lambda/fpl-data-ingester/scripts/dedupe-picks.mjs` scans the whole table, groups by `(season_entry_gw, player_id)`, and for any group with more than one row keeps only the most-recently-synced one (by `last_synced`) and deletes the rest. Supports `--dry-run` and is safe to re-run. Not yet run against production as of this writing — the item count above (6,337) predates that cleanup and should be re-checked once it's been run.

Confirmed via grep that `position_player` is never parsed/split anywhere else in the codebase (only written in `index.mjs`, and echoed through unmodified — never split — by the already-completed one-off `backfill-picks-points.mjs` script), so this was safe to change without touching any read path.

Not yet investigated: whether this same duplicate-row corruption skewed the Advisor's own picks-based logic (`evaluateBenchBoost`, `evaluateChipOptions`, `suggestTransfer`'s owned-player set, all shipped 2026-09 as part of the Tier-1/chip-comparison work above) for any manager who had it — all of those iterate over `picks` assuming one row per player per gameweek, so a manager with duplicate rows would have gotten subtly wrong Advisor output too, not just a visually-broken squad view. Worth a follow-up pass once the cleanup script has actually been run.
