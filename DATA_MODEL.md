...
Partition key `league_id` (N), sort key `season_string` (S). Written only by `scripts/add-league.mjs`; not yet read by any handler (registration is currently step one of a larger multi-league effort, tracked separately — see the multi-league GitHub issue).
...
- **Multi-league support is Phase 1 only so far (2026-08-14):** the `leagues` registry table + onboarding validation (above) exist, but none of `fpl_entry_gameweek` / `fpl_entry_picks` / `fpl_league_standings` / `gw-winners-cache` have `league_id` in their key yet — today's app still implicitly assumes exactly one league's data lives in each. Still to do: (1) add `league_id` into those tables' keys and repartition the ingester/read paths around it, (2) a mid-season backfill job for a newly-registered league (walk its picks/standings back to GW1 of the current season via the live FPL API), (3) frontend URL routing (`epl.candorsolutions.us/{league_id}`, via `react-router` + a Cloudflare Pages `_redirects` catch-all) in place of the season dropdown, (4) Trends' cross-season manager matching, which currently matches on `team_name` alone and only works because exactly one league's data exists today — once multiple leagues coexist it must resolve `league_group_id` first and match `team_name` only within that group's own `league_id`s.
